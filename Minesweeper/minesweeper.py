...
class Board:
    ...
    def dig(self, row, col):
        ...
        # dig at location with neighboring bombs -> recursively dig neighbors
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, (row+1)+1)):
            for c in range(max(0, col-1), min(self.dim_size-1, (col+1)+1)):
                if (r,c) in self.dug:
                    continue #don't dog where you've already dug
                self.dig(r,c)
    ...
    def __str__(self):
        ...
        # return a string that shows tha board to the player
